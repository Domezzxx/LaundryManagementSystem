...
public class ResetPasswordGUI extends JFrame {
    ...
    public ResetPasswordGUI() {
        ...
        // Card Panel (สำหรับสลับหน้า)
        cardPanel = new JPanel(new CardLayout()); // เปลี่ยนจาก JPanel cardPanel
        cardPanel.setBackground(Color.WHITE);

        // Step 1: Generate Token Panel
        tokenPanel = createTokenPanel();

        // Step 2: Reset Password Panel
    ...
    private JPanel createTokenPanel() {
        ...
        tokenDisplayPanel.setVisible(false);
    ...
    private void updatePassword(String email, String newPassword) throws SQLException {
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
            ...
            // อัพเดทสถานะ token ว่าถูกใช้แล้ว
