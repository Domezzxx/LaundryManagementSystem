...
public class OrderManager {
    ...
    /**
     * ค้นหาด้วย Order ID โดยตรง (ใช้เมื่อกรอกตัวเลข 0-100)
     */
    ...
    /**
     * ค้นหาด้วยเบอร์โทรศัพท์ (ใช้เมื่อกรอกเลข 9-10 หลัก)
     */
    ...
    /**
     * ค้นหาทั่วไป (ชื่อ หรือ ID แบบกว้างๆ)
     */
    ...
    /**
     * Helper method: แปลง ResultSet เป็น Order object
     */
    private Order extractOrderFromResultSet(ResultSet rs) throws SQLException {
        ...
        // ดึงชื่อและเบอร์โทรจาก alias ที่กำหนด
        try {
        ...
        // ใช้ booking_date แทน pickup_date (รองรับทั้ง DATE และ DATETIME)
        try {
            // ลองดึงเป็น Timestamp ก่อน (ถ้าเป็น DATETIME)
            Timestamp pickupTimestamp = rs.getTimestamp("booking_date");
            if (pickupTimestamp != null) {
                order.setPickupDate(pickupTimestamp);
            } else {
                // ถ้าไม่ได้เป็น Timestamp ลองดึงเป็น Date
    ...
    /**
     * ดึงประวัติสถานะของ Order
     * @return Map<สถานะ, วันเวลา>
     */
    ...
    /**
     * เพิ่มประวัติสถานะ (ใช้ตอนสร้าง Order ใหม่)
     */
    ...
    /**
     * ดึงประวัติสถานะทั้งหมดพร้อมรายละเอียด
     */
    ...
    /**
     * สร้าง Order ใหม่พร้อมบันทึกประวัติสถานะแรก
     */
    ...
    public boolean updateOrderStatus(String orderId, String newStatus, String changedBy, String remarks) {
        ...
        try {
            ...
            // ✅ แก้ไข Logic: ตัดส่วนที่เพิ่มวันออก ใช้เวลาปัจจุบันทั้งคู่
            ...
            try (PreparedStatement pstmt = conn.prepareStatement(sql.toString())) {
                ...
                if (updateDate) {
                    // ✅ แก้ไข: ใช้ setTimestamp และตัวแปร newTimestamp ที่ถูกต้อง
    ...
    private String buildUpdateQuery(String newStatus) {
        // เริ่มต้น Query (มี ? ตัวที่ 1)
        StringBuilder query = new StringBuilder("UPDATE laundryorder SET status = ?");

        String status = (newStatus == null) ? "" : newStatus.trim();

        // ตรวจสอบเงื่อนไขเพื่อเพิ่ม Logic คอลัมน์ booking_date
        // ข้อควรระวัง: อย่าใส่ WHERE ใน switch case
        switch (status) {
            case "รอดำเนินการ":
            ...
            case "เสร็จสิ้น":
                ...
                break;
            default:
                // กรณีอื่นๆ หรือ ยกเลิก ไม่ต้องทำอะไรกับ booking_date
                break;
        }

        // ✅ ปิดท้ายด้วย WHERE เสมอ (ทำให้มี ? ตัวที่ 2 แน่นอน)
    ...
    /**
     * ลบ Order (ระวัง: จะลบประวัติสถานะด้วย)
     */
    ...
    /**
     * Class เก็บข้อมูลประวัติสถานะแบบละเอียด
     */
