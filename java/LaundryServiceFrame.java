...
public class LaundryServiceFrame extends JFrame {
    ...
    // 🟢 2a. Constructor ใหม่ ที่รับ HomeFrame
    ...
    // 2b. Constructor สำหรับ Main/Testing
    public LaundryServiceFrame(int customerId) {
        this(customerId, null);
    }

    // 2c. Constructor สำหรับ Main/Testing
    ...
    private JPanel createTopBar() {
        ...
        // ✅ 3. ส่ง username ไปที่ UserProfileFrame
        profileItem.addActionListener(e -> {
            // 1. ซ่อนหน้า HomeFrame (เสมือนปิดไป)
            this.setVisible(false);

            // 2. สร้างหน้า UserProfileFrame
            UserProfileFrame profileFrame = new UserProfileFrame(this.username);

            // 3. เพิ่ม Listener: เมื่อปิดหน้า Profile ให้เปิดหน้า Home กลับมา
    ...
    private JPanel createServiceItem(String itemName, String imagePath, int price) {
        ...
        // --- 🟢 ส่วนที่แก้ไข: โหลดรูปภาพแทน Emoji ---
        JLabel iconLabel = new JLabel();

        // ลองโหลดรูปภาพ
        ImageIcon icon = new ImageIcon(imagePath);

        // เช็คว่าโหลดรูปได้จริงหรือไม่ (ความกว้าง > -1 หรือ 0)
        if (icon.getImageLoadStatus() == MediaTracker.COMPLETE && icon.getIconWidth() > 0) {
            // ย่อรูปให้สวยงาม (ขนาด 60x60 px)
            Image img = icon.getImage().getScaledInstance(60, 60, Image.SCALE_SMOOTH);
            iconLabel.setIcon(new ImageIcon(img));
        } else {
            // ⚠️ กรณีหาไฟล์รูปไม่เจอ ให้ใช้ Emoji เดิมแทน
            iconLabel.setText("👕");
    ...
    private void openBookingFrame() {
        ...
        try {
            // 🟢 เรียกใช้ Constructor 6 พารามิเตอร์ ของ BookingFrame
            BookingFrame bookingFrame = new BookingFrame(
                    ...
                    customerId,
                    this.homeFrame,
                    this // ส่ง LaundryServiceFrame ตัวเองไปด้วย
            );
    ...
    private void addToOrder(String itemName, String serviceType, int quantity, int pricePerItem) {
        // ต้องมั่นใจว่า BookingFrame.OrderItem สามารถเรียกใช้ได้
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            // ในการทดสอบตรงนี้ จะไม่มี HomeFrame จึงส่ง null ไป
