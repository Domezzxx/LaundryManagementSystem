...
/**
 * Data Access Object สำหรับจัดการข้อมูล Order ในฐานข้อมูล
 * รวมเมธอดเดิมและเมธอดใหม่สำหรับหน้าประวัติการใช้บริการ
 * *** แก้ไข: รองรับการ parse booking_time แบบช่วงเวลา (08:00-10:00) ***
 */
public class OrderDataAccessObject {
    ...
    /**
     * ดึงรายการซักผ้า (order_details) สำหรับ Order ID ที่กำหนด
     * คืนค่าเป็น List<String>
     */
    ...
    /**
     * อัปเดตรายการซักผ้าและราคารวมในฐานข้อมูล
     * ทำการอัปเดตในตาราง order_detail และตารางหลัก laundryorder
     */
    ...
    /**
     * แปลง String เป็น LocalDate โดยรองรับหลายรูปแบบ
     * *** แก้ไข: เพิ่มการตรวจสอบ LocalDateTime หากสตริงมี Time Stamp ติดมา ***
     */
    private LocalDate parseBookingDate(String dateString) {
        ...
        // 1. ตรวจสอบรูปแบบที่มี Time Stamp ก่อน (ถ้าฐานข้อมูลส่ง DATETIME มา)
        try {
            // yyyy-MM-dd HH:mm:ss หรือ yyyy-MM-dd HH:mm
            LocalDateTime dateTime = LocalDateTime.parse(dateString, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
            return dateTime.toLocalDate();
        } catch (DateTimeParseException e) {
            // ไม่ใช่รูปแบบ Time Stamp, ทำต่อ
        }
    ...
    /**
     * แปลง String เป็น LocalTime โดยรองรับหลายรูปแบบ
     * *** แก้ไข: เพิ่มการรองรับรูปแบบช่วงเวลา (เช่น "08:00-10:00") ***
     * *** แก้ไข: ใช้ Locale.ENGLISH สำหรับ AM/PM เพื่อแก้ไข Warnings ***
     */
    private LocalTime parseBookingTime(String timeString) {
        ...
        // 🟢 ตรวจสอบว่าเป็นรูปแบบช่วงเวลาหรือไม่ (เช่น "08:00-10:00")
        if (timeString.contains("-")) {
            // แยกเอาเฉพาะเวลาเริ่มต้น
            String[] parts = timeString.split("-");
            if (parts.length > 0) {
                timeString = parts[0].trim();
                // ไม่แสดง info message เพราะเป็นพฤติกรรมปกติ
            }
        }
        ...
        DateTimeFormatter[] formatters = {
                // รูปแบบ 12-ชั่วโมง (ต้องใช้ Locale)
                TIME_12H_FORMATTER,     // 08:28 AM (hh:mm a)
                TIME_H_MM_A_FORMATTER,  // 8:28 AM (h:mm a)
                TIME_H_MM_SS_A_FORMATTER,// 8:28:00 AM (h:mm:ss a)

                // รูปแบบ 24-ชั่วโมง (ไม่ต้องใช้ Locale)
                DateTimeFormatter.ofPattern("HH:mm:ss"),    // 20:28:00
                DateTimeFormatter.ofPattern("HH:mm"),       // 20:28
                DateTimeFormatter.ofPattern("H:mm"),        // 8:28
        };
        ...
        // แสดง warning เฉพาะกรณีที่ parse ไม่สำเร็จจริงๆ
        System.err.println("Warning: Could not parse time: " + timeString);
        return null;
    }

    /**
     * ดึงรายการ Orders ทั้งหมดของลูกค้าที่ระบุ
     * สำหรับหน้าประวัติการใช้บริการ
     */
    ...
    /**
     * ดึงรายการ OrderDetail objects สำหรับ Order ID ที่กำหนด
     * คืนค่าเป็น List<OrderDetail>
     */
    ...
    /**
     * ดึงข้อมูล Order เดียวตาม Order ID
     */
