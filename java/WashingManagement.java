...
public class WashingManagement extends JFrame {
    ...
    private JPanel createSearchPanel() {
        ...
        // Filter ComboBox
        JLabel lblFilter = new JLabel("กรอง:");
    ...
    // Public method to reload data from database (called from Dialog)
    ...
    private void filterMachines() {
        ...
        for (Machine machine : machines) {
            // ✅ แปลงสถานะเป็นภาษาไทยก่อนเช็ค
            String displayStatus = convertStatusToThai(machine.status);

            // ✅ Check search text match - ค้นหาแบบตรงทั้งคำ (exact match) ทุกฟิลด์
            boolean matchSearch = false;

            if (searchText.isEmpty()) {
                matchSearch = true;
            } else {
                // ค้นหา ID แบบตรงทั้งคำ
                if (machine.id.toLowerCase().equals(searchText)) {
                    matchSearch = true;
                }
                // ค้นหาประเภทแบบตรงทั้งคำ
                else if (machine.type.toLowerCase().equals(searchText)) {
                    matchSearch = true;
                }
                // ค้นหาสถานะแบบตรงทั้งคำ (ทั้งภาษาไทยและอังกฤษ)
            ...
            // Add row if both conditions match
        ...
        // Update counts (based on all data, not filtered)
        updateStatusCounts();
    }
