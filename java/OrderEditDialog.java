...
public class OrderEditDialog extends JDialog {
    ...
    private void initComponents() {
        ...
        // ✅ 1. เพิ่ม Logic เมื่อเปลี่ยนสถานะ (Action Listener)
        cmbStatus.addActionListener(e -> {
            if (isInitializing) return; // ข้ามถ้ากำลังโหลดข้อมูลครั้งแรก

            String status = (String) cmbStatus.getSelectedItem();
            if ("เสร็จสิ้น".equals(status)) {
                // ถ้าเลือกเสร็จสิ้น -> ใส่วันเวลาปัจจุบัน
                txtPickupDate.setText(dateFormat.format(new Date()));
            } else {
                // ถ้าเลือกสถานะอื่น -> ใส่ขีด -
    ...
    private void loadOrderData() {
        ...
        if (order != null) {
            ...
            // ✅ 2. ตั้งค่าเริ่มต้นของช่องวันที่
            // ถ้าสถานะเป็น "เสร็จสิ้น" ให้โชว์วันที่จาก DB (ถ้ามี)
            // ถ้าสถานะอื่น ให้โชว์ "-"
    ...
    // ... (ส่วน loadTableFromDB, getBookingTimeFromDB, calculatePrice คงเดิม) ...
    ...
    private void onSave(ActionEvent e) {
        try {
            ...
            // บันทึกรายละเอียด, ราคา, สถานะ และวันที่รับผ้า (ถ้ามี)
            // หมายเหตุ: orderDAO.updateOrderDetailsAndPrice ปกติอัปเดตแค่รายละเอียด
            // คุณอาจต้องเพิ่ม logic อัปเดตวันที่รับผ้าใน OrderManager หรือ OrderDAO ด้วยถ้าต้องการให้วันที่เซฟลง DB
            ...
            // ✅ เพิ่ม: ถ้าสถานะเสร็จสิ้น ให้อัปเดตวันที่รับผ้าลง DB ด้วย (ถ้าจำเป็น)
            // ตรงนี้ขึ้นอยู่กับว่า OrderManager ของคุณจัดการวันที่รับผ้าตอน updateOrderStatus หรือไม่
            // ถ้าไม่ คุณอาจต้องเพิ่มคำสั่ง SQL update laundryorder SET pickup_date = NOW() WHERE order_id = ...
